...
public class RentalItemType implements ItemType {
	...
	public void addRentalItem(String name, MyDate lastMaintainedDate,
			double monthsBtwMaintenance) {
		RentalItem hireItem = new RentalItem(this, name, lastMaintainedDate,
				monthsBtwMaintenance);
		hireItems.put(name, hireItem);
	}
	...
	public boolean hasHiresToReturn(int count) {
		for (Iterator it = hireItems.values().iterator(); it.hasNext()
				&& count > 0;) {
			if (((RentalItem) it.next()).hasHireToReturn())
				count--;
		}
		return count == 0;
	}
	public List<RentalItem> getHiresToReturn(int count) {
		...
		for (int i = 0; i < count; i++) {
			...
			for (Iterator it = hireItems.values().iterator(); it.hasNext();) {
				RentalItem hireItem = (RentalItem) it.next();
	...
	public Money calculateRefundOnReturn(MyDate returnDate, int count) {
	...
	public List<RentalItem> reserveFor(Reservation reservation, int count,
		...
		for (Iterator it = hireItems.values().iterator(); it.hasNext()
			...
			RentalItem item = (RentalItem) it.next();
	...
	public boolean removeForRepair(int count) {
		for (Iterator it = hireItems.values().iterator(); it.hasNext()
				&& count > 0; count--) {
			((RentalItem) it.next()).repair();
		}
		if (count != 0) {
			return false;
		}
	...
	public boolean returnFromRepair(MyDate date, int count) {
		for (Iterator it = hireItems.values().iterator(); it.hasNext()
				&& count > 0;) {
			RentalItem hireItem = (RentalItem) it.next();
	...
	public boolean checkNoOfItemsInRepair(int count) {
		int no = 0;
		for (Iterator it = hireItems.values().iterator(); it.hasNext();) {
			if (((RentalItem) it.next()).isBeingRepaired())
				no++;
		}
		return no >= count;
	}
	...
	public void changeDetails(Rates rates, Money bond) {
		//FIXME: count and item name is uneditable for now, because it can cause
		//conflicts with rentals
//		if(count < getCount()) {
//			for (int i = 0; i < getCount()-count; i++) {
//				for (Iterator iter = hireItems.keySet().iterator(); iter.hasNext();) {
//					String name = (String) iter.next();
//					if(hireItems.get(name).isFree()) {
//						hireItems.remove(name);
//						break;
//					}
//				}
//			}
//		} else if (count > getCount()) {
//			for (; absoluteCount < count-getCount()+absoluteCount; absoluteCount++) {
//				addRentalItem(name + absoluteCount);
//			}
//		}
		this.rates = rates;
		this.deposit = bond;
