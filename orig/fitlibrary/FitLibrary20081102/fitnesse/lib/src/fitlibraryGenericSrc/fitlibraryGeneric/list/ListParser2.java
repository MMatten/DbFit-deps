...
public class ListParser2 extends ListParser {
    ...
    @Override
	protected List<?> parseTable(Table table, TestResults testResults) {
    	...
    	case CLASS_TYPE:
       		if (componentTyped.isEffectivelyPrimitive() || componentTyped.isEnum()) {
    			...
    			setUpTraverse.setDynamicVariables(evaluator.getDynamicVariables());
	...
	private List<?> parseNested(Table table, TestResults testResults) {
    ...
    protected boolean tableMatches(Table table, Object result, TestResults testResults) {
    	...
    	case PARAMETERIZED_TYPE:
    		if (result.getClass().isArray())
    			result = Arrays.asList((Object[])result);
    		NestingListTraverse nestingList = new NestingListTraverse((List)result,componentTyped);
