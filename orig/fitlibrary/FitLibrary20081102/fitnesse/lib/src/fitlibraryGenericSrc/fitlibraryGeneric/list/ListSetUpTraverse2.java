...
public class ListSetUpTraverse2 extends DoTraverse {
	...
	public Object interpretAfterFirstRow(Table table, TestResults testResults) {
        try {
            ...
            if (classColumn < 0) {
                Object element = ClassUtility.newInstance(componentType);
    ...
    private void bindFirstRowToTargetsForObject(Object element, Row firstRow, Row row, TestResults testResults) {
        ...
        for (int i = 0; i < firstRow.size(); i++) {
            ...
            if (!DomainObjectSetUpTraverse.givesClass(cell,this)) {
                try {
                	targets[i] = LookupMethodTarget.findSetter(cell.text(this), this);
                } catch (Exception e) {
    ...
    private Object createElement(Row firstRow, int classColumn, Row row, TestResults testResults) throws InstantiationException, IllegalAccessException, SecurityException, IllegalArgumentException, NoSuchMethodException, InvocationTargetException {
    	if (classColumn < 0)
    		return ClassUtility.newInstance(componentType);
    	String typeName = row.text(classColumn,this);
    	Class<?> findClass = LookupMethodTarget.findClassFromFactoryMethod(this,componentType,typeName);
    	if (findClass == null)
    	...
    	try {
    		Object sut = ClassUtility.newInstance(findClass);
