...
public class RentalItem {
	...
	public void doHire(Rental hire, Reservation reservation, Client client, StaffMember staffMember) {
		reservations.remove(reservation.getStartDate());
		currentRental = hire;
	}
	...
	public boolean reserveFor(Reservation reservation, MyDate startDate, MyDate dueDate) {
		...
		for(Enumeration pendingHireEnum = reservations.keys(); pendingHireEnum.hasMoreElements(); ) {

			MyDate pendingHireStartDate = (MyDate)pendingHireEnum.nextElement();
			MyDate pendingHireDueDate = ((Reservation)reservations.get(pendingHireStartDate)).getDueDate();
