...
public class ListTraverse extends CollectionTraverse {
	...
	public void interpret(Row row, List remainingActuals, TestResults testResults) throws Exception {
        if (remainingActuals.isEmpty()) {
            row.missing(testResults);
            return;
        }
        int rowLength = row.size();
        MethodTarget[] columnBindings = (MethodTarget[])remainingActuals.get(0);
